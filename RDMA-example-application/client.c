...
int main(int argc, char *argv[]) {
  ...
  if (event->event != RDMA_CM_EVENT_ADDR_RESOLVED) return 1;
  ...
  if (event->event != RDMA_CM_EVENT_ROUTE_RESOLVED) return 1;
  ...
  if (event->event != RDMA_CM_EVENT_ESTABLISHED) return 1;
  ...
  while (1) {
    ...
    if (ibv_poll_cq(cq, 1, &wc) != 1) return 1;
