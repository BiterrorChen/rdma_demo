...
RdmaSocket::RdmaSocket(int _cqNum, uint64_t _mm, uint64_t _mmSize,
                       Configuration *_conf, bool _isServer, uint8_t _Mode)
    : DeviceName(NULL),
      Port(1),
      ServerPort(5678),
      GidIndex(0),
  ...
  for (int i = 0; i < cqNum; i++) cq[i] = NULL;
...
void RdmaSocket::NotifyPerformance() {
  for (int i = 0; i < WORKER_NUMBER; i++) {
    printf("\n");
    Debug::notifyInfo("TotalWriteSize = %ld, WriteTimeCost = %ld", WriteSize[i],
                      WriteTimeCost[i]);
    Debug::notifyInfo("TotalReadSize = %ld, ReadTimeCost = %ld", ReadSize[i],
                      ReadTimeCost[i]);
  }
}
...
bool RdmaSocket::CreateResources() {
  ...
  mrFlags = IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ |
...
CreateResourcesExit:
  if (rc) {
    ...
    for (i = 0; i < cqNum; i++) {
      if (cq[i] != NULL) ibv_destroy_cq(cq[i]);
    }
    if (pd) {
      ibv_dealloc_pd(pd);
      pd = NULL;
...
bool RdmaSocket::ConnectQueuePair(PeerSockData *peer) {
  ...
  /* Change NodeID first */
  if (DataSyncwithSocket(peer->sock, sizeof(ExchangeID), (char *)&LocalID,
                         (char *)&RemoteID) < 0) {
  ...
  if (!isServer || (isServer && peer->NodeID > conf->getServerCount())) {
    ...
    DoubleQP = true;
    for (int i = 2; i < QP_NUMBER; i++) CreateQueuePair(peer, i);
  }
  if (GidIndex >= 0) {
    rc = ibv_query_gid(ctx, Port, GidIndex, &MyGid);
    if (rc) {
      Debug::notifyError("could not get gid for port: %d, index: %d", Port,
                         GidIndex);
      return false;
    }
  } else {
  ...
  if (DataSyncwithSocket(peer->sock, sizeof(ExchangeMeta), (char *)&LocalMeta,
                         (char *)&RemoteMeta) < 0) {
    Debug::notifyError("failed to exchange connection data between sides");
    rc = 1;
    goto ConnectQPExit;
  ...
  for (int i = 0; i < QP_NUMBER; i++) peer->qpNum[i] = RemoteMeta.qpNum[i];
...
int RdmaSocket::DataSyncwithSocket(int sock, int size, char *LocalData,
                                   char *RemoteData) {
  int rc;
  int readBytes = 0;
...
void RdmaSocket::SyncTool(uint16_t NodeID) {
  while (peers[NodeID] == NULL) usleep(100000);
...
void RdmaSocket::RdmaAccept(int sock) {
  ...
  while (isRunning &&
         (fd = accept(sock, (struct sockaddr *)&RemoteAddress, &sin_size)) !=
             -1) {
...
int RdmaSocket::SocketConnect(uint16_t NodeID) {
  ...
  inet_aton(conf->getIPbyID(NodeID).c_str(),
            (struct in_addr *)&RemoteAddress.sin_addr);
  ...
  int t = 3;
  while (t >= 0 &&
         connect(sock, (struct sockaddr *)&RemoteAddress,
...
*/
bool RdmaSocket::RdmaSend(uint16_t NodeID, uint64_t SourceBuffer,
                          uint64_t BufferSize) {
  // assert(peers[NodeID]);
  struct ibv_sge sg;
...
bool RdmaSocket::_RdmaBatchSend(uint16_t NodeID, uint64_t SourceBuffer,
                                uint64_t BufferSize, int BatchSize) {
  assert(peers[NodeID]);
  ...
  for (w_i = 0; w_i < BatchSize; w_i++) {
    ...
    send_wr[w_i].send_flags =
        (peer->counter & SIGNAL_BATCH) == 0 ? IBV_SEND_SIGNALED : 0;
...
bool RdmaSocket::RdmaReceive(uint16_t NodeID, uint64_t SourceBuffer,
                             uint64_t BufferSize) {
  // assert(peers[NodeID]);
  struct ibv_sge sg;
...
bool RdmaSocket::_RdmaBatchReceive(uint16_t NodeID, uint64_t SourceBuffer,
                                   uint64_t BufferSize, int BatchSize) {
...
bool RdmaSocket::RdmaRead(uint16_t NodeID, uint64_t SourceBuffer,
                          uint64_t DesBuffer, uint64_t BufferSize, int TaskID) {
  // assert(peers[NodeID]);
  struct ibv_sge sg;
  struct ibv_send_wr wr;
  struct ibv_send_wr *wrBad;
...
bool RdmaSocket::_RdmaBatchRead(uint16_t NodeID, uint64_t SourceBuffer,
                                uint64_t DesBuffer, uint64_t BufferSize,
                                int BatchSize) {
  // assert(peers[NodeID]);
  struct ibv_sge sgl[MAX_POST_LIST];
  struct ibv_send_wr send_wr[MAX_POST_LIST];
  ...
  for (w_i = 0; w_i < BatchSize; w_i++) {
    ...
    send_wr[w_i].send_flags =
        (peer->counter & SIGNAL_BATCH) == 0 ? IBV_SEND_SIGNALED : 0;
    send_wr[w_i].wr.rdma.remote_addr =
        DesBuffer + peer->RegisteredMemory;  // + w_i * 4096;
...
bool RdmaSocket::RemoteRead(uint64_t bufferSend, uint16_t NodeID,
                            uint64_t bufferReceive, uint64_t size) {
  int shipSize;
...
bool RdmaSocket::DataTransferWorker(int id) {
  ...
  while (true) {
    ...
    if (task->OpType) {
      /* Write opration. */
      OutboundHamal(id, task->bufferSend, task->NodeID, task->bufferReceive,
                    task->size);
    } else {
      InboundHamal(id, task->bufferSend, task->NodeID, task->bufferReceive,
                   task->size);
    }
  }
  return true;
...
bool RdmaSocket::InboundHamal(int TaskID, uint64_t bufferSend, uint16_t NodeID,
                              uint64_t bufferReceive, uint64_t size) {
  ...
  while (TotalSizeSend < size) {
    SendSize = (size - TotalSizeSend) >= SendPoolSize ? SendPoolSize
                                                      : (size - TotalSizeSend);
    ...
    //                1);
    gettimeofday(&start, NULL);
    RdmaRead(NodeID, SendPoolAddr, bufferReceive + TotalSizeSend, SendSize,
             TaskID + 1);
    PollCompletion(NodeID, 1, &wc);
    memcpy((void *)(bufferSend + TotalSizeSend), (void *)SendPoolAddr,
           SendSize);
    gettimeofday(&end, NULL);
...
bool RdmaSocket::RdmaWrite(uint16_t NodeID, uint64_t SourceBuffer,
                           uint64_t DesBuffer, uint64_t BufferSize,
                           uint32_t imm, int TaskID) {
  // assert(peers[NodeID]);
  struct ibv_sge sg;
  struct ibv_send_wr wr;
  struct ibv_send_wr *wrBad;
...
bool RdmaSocket::RemoteWrite(uint64_t bufferSend, uint16_t NodeID,
                             uint64_t bufferReceive, uint64_t size) {
  int shipSize;
  TransferTask tasks[4];
  if (size < 4 * 1024 * 1024) {
...
bool RdmaSocket::OutboundHamal(int TaskID, uint64_t bufferSend, uint16_t NodeID,
                               uint64_t bufferReceive, uint64_t size) {
  uint64_t SendPoolSize = 1024 * 1024;
  uint64_t SendPoolAddr = mm + 4 * 1024 + TaskID * 1024 * 1024;
  uint64_t TotalSizeSend = 0;
  ...
  while (TotalSizeSend < size) {
    SendSize = (size - TotalSizeSend) >= SendPoolSize ? SendPoolSize
                                                      : (size - TotalSizeSend);
    gettimeofday(&start, NULL);
    memcpy((void *)SendPoolAddr, (void *)(bufferSend + TotalSizeSend),
           SendSize);
    ...
    //                (uint32_t)-1,
    //                1);
    RdmaWrite(NodeID, SendPoolAddr, bufferReceive + TotalSizeSend, SendSize, -1,
              TaskID + 1);
    PollCompletion(NodeID, 1, &wc);
    // if (SendSize > 32 * 1024) {
    ...
    if (WriteTest) {
      gettimeofday(&start, NULL);
      for (int i = 0; i < 10; i++) {
        RdmaWrite(NodeID, SendPoolAddr, bufferReceive, 1024 * 1024, -1,
                  TaskID + 1);
        PollCompletion(NodeID, 1, &wc);
      }
      gettimeofday(&end, NULL);
    ...
    Debug::debugItem("Source Addr = %lx, Des Addr = %lx, Size = %d",
                     SendPoolAddr, bufferReceive + TotalSizeSend, SendSize);
    TotalSizeSend += SendSize;
  }
  __sync_fetch_and_add(&TransferSignal, 1);
  return true;
...
bool RdmaSocket::_RdmaBatchWrite(uint16_t NodeID, uint64_t SourceBuffer,
                                 uint64_t DesBuffer, uint64_t BufferSize,
                                 uint32_t imm, int BatchSize) {
  // assert(peers[NodeID]);
  struct ibv_sge sgl[MAX_POST_LIST];
  struct ibv_send_wr send_wr[MAX_POST_LIST];
  struct ibv_send_wr *wrBad;
  ...
  // peer->qp->qp_num, peer->cq, peer->rkey);
  for (w_i = 0; w_i < BatchSize; w_i++) {
    if ((peer->counter & SIGNAL_BATCH) == 0 && peer->counter > 0 && !isServer) {
      PollCompletion(NodeID, 1, &wc);
    }
    ...
    send_wr[w_i].wr.rdma.remote_addr =
        DesBuffer + peer->RegisteredMemory + w_i * 4096;
    Debug::debugItem("remote address = %lx, Counter = %d, imm = %lx",
                     send_wr[w_i].wr.rdma.remote_addr, peer->counter, imm);
    send_wr[w_i].wr.rdma.rkey = peer->rkey;
    peer->counter += 1;
  }
...
bool RdmaSocket::RdmaFetchAndAdd(uint16_t NodeID, uint64_t SourceBuffer,
                                 uint64_t DesBuffer, uint64_t Add) {
  // assert(peers[NodeID]);
  struct ibv_sge sg;
  struct ibv_send_wr wr;
  struct ibv_send_wr *wrBad;
  ...
  wr.wr.atomic.compare_add =
      Add; /* value to be added to the remote address content */
...
bool RdmaSocket::RdmaCompareAndSwap(uint16_t NodeID, uint64_t SourceBuffer,
                                    uint64_t DesBuffer, uint64_t Compare,
                                    uint64_t Swap) {
  // assert(peers[NodeID]);
  struct ibv_sge sg;
  struct ibv_send_wr wr;
  ...
  wr.wr.atomic.swap =
      Swap; /* the value that remote address will be assigned to */
...
int RdmaSocket::PollCompletion(uint16_t NodeID, int PollNumber,
                               struct ibv_wc *wc) {
  int count = 0;
...
int RdmaSocket::getCQCount() { return cqNum; }

uint16_t RdmaSocket::getNodeID() { return MyNodeID; }
void RdmaSocket::WaitClientConnection(uint16_t NodeID) {
  while (peers[NodeID] == NULL) usleep(1);
}
